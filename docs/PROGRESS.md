...
| **O2.S1+S2** 规则引擎 + 谓词下推 | `_next_` | `internal/optimizer/rules/`（RewriteRule + Engine 不动点 + PredicatePushdown）+ CatalogStatsReader | **首个优化生效**：`where` 推过 extend/project 到 source（语义等价，sqlite+pg e2e 验证；11 单测） |
